...
unsigned long rstart_time;
unsigned long rcurrent_time;
unsigned long relapsed_time;
unsigned long rsensor_read;
...
class LineSensor_c {
  public:
    ...
    void Rightsidesensor_read(){
      pinMode(LS_SIDER_IN_PIN, OUTPUT);
      digitalWrite(LS_SIDER_IN_PIN, HIGH);
      delayMicroseconds(10);
      rstart_time=micros();
      while (digitalRead(LS_SIDER_IN_PIN)==HIGH) {
        
      }
      rcurrent_time=micros();
      relapsed_time= rcurrent_time-rstart_time;
      rsensor_read=relapsed_time;  
